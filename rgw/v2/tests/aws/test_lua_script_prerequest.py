...
"""
    ...
    if lua_object_lock_mode and "x-amz-bucket-object-lock-enabled" not in retrieved_script:
        raise AssertionError(
        ...
        for bc in range(config.bucket_count):
            ...
            if lua_object_lock_mode:
                ...
                ver_out = utils.exec_shell_cmd(
                    f"aws s3api get-bucket-versioning --bucket {bucket_name} --endpoint-url {endpoint}"
                )
                ...
                lock_out = utils.exec_shell_cmd(
                    f"aws s3api get-object-lock-configuration --bucket {bucket_name} --endpoint-url {endpoint}"
                )
                ...
                if config.test_ops.get("put_default_retention"):
                    ...
                    try:
                        # 1) Object-level put-object-retention (doc: upload then put-object-retention then get)
                        aws_reusable.put_object(
                            cli_aws, bucket_name, key_early, endpoint
                        )
                        utils.exec_shell_cmd(
                            f"aws s3api put-object-retention --bucket {bucket_name} --key {key_early} "
                            f"--endpoint-url {endpoint} --retention "
                            f'\'{{"Mode":"{mode}","RetainUntilDate":"{retain_until}"}}\''
                        )
                        ret_early = utils.exec_shell_cmd(
                            f"aws s3api get-object-retention --bucket {bucket_name} --key {key_early} --endpoint-url {endpoint}"
                        )
                        if "Retention" not in ret_early or "RetainUntilDate" not in ret_early:
                            raise AssertionError(
                        ...
                        # 2) Bucket default: put-object-lock-configuration (Rule/DefaultRetention)
                        lock_cfg = (
                        ...
                        utils.exec_shell_cmd(
                            f"aws s3api put-object-lock-configuration --bucket {bucket_name} "
                            f"--endpoint-url {endpoint} --object-lock-configuration '{lock_cfg}'"
                        )
                        log.info(f"Set default retention {mode} {days} days on {bucket_name}")
                        # 3) Verify get-object-lock-configuration returns Rule (doc: Rule/DefaultRetention)
                        ...
                        # 4) Upload object with default retention, get-object-retention
                        if config.test_ops.get("test_multipart", False):
                            config.obj_size = config.objects_size_range["max"]
                            log.info("Uploading object with default retention via multipart: %s", key_name)
                        ...
                        ret_out = utils.exec_shell_cmd(
                            f"aws s3api get-object-retention --bucket {bucket_name} --key {key_name} --endpoint-url {endpoint}"
                        )
                        if "Retention" not in ret_out:
                            log.warning(
                                f"get-object-retention for {key_name} (may be NoneType on some versions): {ret_out}"
                            )
                        # 5) delete-object without version-id → DeleteMarker (doc step)
                        del_out = utils.exec_shell_cmd(
                            f"aws s3api delete-object --bucket {bucket_name} --key {key_name} --endpoint-url {endpoint}"
                        )
                        log.info("aws s3api delete-object (no version-id) output: %s", del_out)
                        if "DeleteMarker" not in str(del_out):
                            log.warning(
                                f"delete-object (no version-id) expected DeleteMarker: {del_out}"
                            )
                        else:
                            log.info("delete-object without version-id returned DeleteMarker as expected")
                        # 6) delete-object by version-id (locked version) → forbidden
                        list_ver = utils.exec_shell_cmd(
                            f"aws s3api list-object-versions --bucket {bucket_name} --endpoint-url {endpoint} "
                        ...
                        if ver_list and ver_list[0].get("VersionId"):
                            ...
                            err = utils.exec_shell_cmd(
                                f"aws s3api delete-object --bucket {bucket_name} --key {key_name} "
                                f"--version-id {vid} --endpoint-url {endpoint}",
                                return_err=True,
                            )
                            log.info("aws s3api delete-object (version-id %s, locked) output: %s", vid, err)
                            if err and (
                        ...
                        # 7) delete by version-id on object-level retention key → forbidden
                        ...
                        if early_list and early_list[0].get("VersionId"):
                            ...
                            err_early = utils.exec_shell_cmd(
                                f"aws s3api delete-object --bucket {bucket_name} --key {key_early} "
                                f"--version-id {vid_early} --endpoint-url {endpoint}",
                                return_err=True,
                            )
                            log.info("aws s3api delete-object (version-id %s, object-level retention) output: %s", vid_early, err_early)
                            if err_early and (
