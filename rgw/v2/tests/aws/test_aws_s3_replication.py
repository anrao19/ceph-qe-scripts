        ...
        for each_user in user_info:
            ...
            if utils.is_cluster_multisite():
                ...
                for bkt in buckets:
                    ...
                    if config.test_ops.get("create_object", False):
                        ...
                        for oc, size in list(config.mapped_sizes.items()):
                            ...
                            utils.exec_shell_cmd(f"fallocate -l {size} {s3_object_name}")
                            log.info(f"upload s3 object: {s3_object_name}")
                            aws_reusable.put_object(cli_aws, bkt.name, s3_object_name, endpoint)
                            if config.test_ops.get("enable_version", False):
                                aws_reusable.put_object(cli_aws, bkt.name, s3_object_name, endpoint)
                        ...
                        if config.test_ops.get("write_io_verify_another_site", False):
                            ...
                            cmd_output = json.loads(aws_auth.run_remote_cmd(
                                rgw_ssh_con, f"radosgw-admin bucket stats --bucket {bkt.name}")
                            )
                            num_objects = (
                            ...
                            aws_path = aws_auth.run_remote_cmd(rgw_ssh_con, "command -v aws")
                            aws_cli = f"{aws_path} s3api"
                            for oc, size in list(config.mapped_sizes.items()):
                                config.obj_size = size
                                s3_object_name = f"new-object-oc-{config.obj_size}"
                                rgw_ssh_con.exec_command(f"fallocate -l {size} {s3_object_name}")
                                log.info(f"upload s3 object: {s3_object_name}")
                            ...
                            re_cmd_output = json.loads(aws_auth.run_remote_cmd(
                                rgw_ssh_con, f"radosgw-admin bucket stats --bucket {bkt.name}")
                            )
                ...
                for bkt in buckets:
                    ...
                    aws_reusable.delete_bucket_s3_replication(cli_aws, bkt.name, endpoint)
...
if __name__ == "__main__":
    ...
    try:
        ...
        parser = argparse.ArgumentParser(description="RGW s3 bucket replication through AWS")
        parser.add_argument(
